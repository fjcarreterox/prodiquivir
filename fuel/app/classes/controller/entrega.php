...
class Controller_Entrega extends Controller_Template
{
    ...
    public function action_report(){
         ...
         $this->template->title = "Informe global de la campaña 2020";
    ...
    public function action_list($idpuesto = null){
        ...
        if(is_null($idpuesto)) {
			$data['titulo'] = "durante la campaña 2020.";
            $data['entregas'] = Model_Entrega::find('all', array('where'=>array(array('fecha','>','2019-12-31')),'order_by' => array('Fecha' => 'desc'),'order_by' => array('id' => 'desc')));
	...
	public function action_create(){
		if (Input::method() == 'POST'){
			...
			if ($val->run()){
                $albs=Model_Albaran::find('all',array('where'=>array(array('fecha','LIKE','2020%')),'order_by'=>array('id'=>'desc','idalbaran'=>'desc')));
                ...
                if(!$last_albaran) {
                    ...
                    $last_albaran_num = Model_Albaran::query()->where('fecha','LIKE','2020%')->max('idalbaran');
                }
